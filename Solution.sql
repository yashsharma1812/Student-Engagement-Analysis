...
-- Calculate the average number of days between a student's registration and their first content watch
    ROUND(SUM(days_diff_reg_watch) / COUNT(first_date_watched),2) AS av_reg_watch,
    
-- Calculate the average number of days between a student's first content watch and their first purchase
    ROUND(SUM(days_diff_watch_purch) / COUNT(first_date_purchased),2) AS av_watch_purch
FROM
...
DATEDIFF(MIN(e.date_watched),date_registered) AS days_diff_reg_watch,
...
HAVING first_date_watched <= first_date_purchased OR days_diff_watch_purch IS NULL) a;
